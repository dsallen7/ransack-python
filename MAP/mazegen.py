    ...
    def getRandomTile(self, type):
        if type == 'open':
            ...
            while not ((self.getMatrixEntry(x, y - 1),
                   ...
                   ) in list(itertools.permutations(
                   [True, True, True, False]))
                   and not self.getMatrixEntry(x, y)):
                x = randrange(1, self.DIM - 1)
                y = randrange(1, self.DIM - 1)
            return (x, y)
        ...
        for i in range(1, self.DIM - 1):
            ...
            for j in range(1, self.DIM - 1):
                if self.getMatrixEntry(i, j):
                    self.map.setEntry(i, j,
                        mapScr.wallDict[(self.getMatrixEntry(i, j - 1),
                        self.getMatrixEntry(i - 1, j),
                        self.getMatrixEntry(i + 1, j),
                        self.getMatrixEntry(i, j + 1))])
                else:
                    self.map.setEntry(i, j, 0)
    ...
    def generateMap(self):
        ...
        while i < 5:
            ...
            if tile is None:
                ...
                if self.rollDie(0, 2):
                    chestItems.append((13, choice(
                        range(15, 50) + range(15, 30))))
