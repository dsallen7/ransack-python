    ...
    if not room1.secret:
        for i in range( xpos+1, xpos+xD ): # (i, ypos-1) - location of room being considered to add as neighbors
            if gen.map.getEntry( i, ypos ) == const.EWWALL and gen.map.getEntry( i, ypos-1 ) == const.EWWALL and not nFlag:
                    if gen.getRoomByNo( gen.map.getRoomN(i, ypos-1)) is not room2 and not gen.getRoomByNo( gen.map.getRoomN(i, ypos-1)).secret: # to the north
                    ...
                    if gen.getRoomByNo( gen.map.getRoomN(i, ypos+yD+1) ) is not room2 and not gen.getRoomByNo( gen.map.getRoomN(i, ypos+yD+1) ).secret: # south
        ...
        for i in range( ypos+1, ypos+yD ):
            if gen.map.getEntry( xpos, i ) == const.NSWALL and gen.map.getEntry( xpos-1, i ) == const.NSWALL and not eFlag:
                    if gen.getRoomByNo( gen.map.getRoomN(xpos-1, i) ) is not room2 and not gen.getRoomByNo( gen.map.getRoomN(xpos-1, i) ).secret: # east
                    ...
                    if gen.getRoomByNo( gen.map.getRoomN(xpos+xD+1, i)) is not room2 and not gen.getRoomByNo( gen.map.getRoomN(xpos+xD+1, i)).secret: # west
