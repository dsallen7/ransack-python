...
class npc(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, message, imgFile):
        ...
        self.setRect(x * const.blocksize, y * const.blocksize,
    ...
    def move(self, dir, map, heroPos):
        ...
        self.image = self.images[self.imgIdx]
        if map.getEntry(sX + mX, sY + mY) in range(25) and (
                sX + mX, sY + mY) != (hX, hY) and not map.isOccupied(
                sX + mX, sY + mY):
    ...
    def interact(self, hud):
        if not self.Director.getEvent(0):
            if hud.npcDialog(self.message, self.images[8]):
                hud.npcMessage("Excellent news! Here is the key you will need"
                    " to access the Dungeon.", self.images[8])
                self.Director.setEvent(0)
                return ('item', const.KEY)
            else:
                hud.npcMessage("I am most dismayed to hear this; please"
                    " return if you have a change of heart!", self.images[8])
                return None
        else:
            if self.Director.getEvent(11):
                ...
                hud.npcMessage("Haste, young hero. Time is of the essence!",
                    self.images[8])
                return None
...
class Enemy(npc):
    ...
    def seek(self, map, heroPos):
        (sX, sY) = self.getXY()
        (hX, hY) = heroPos
        if misc.DistanceX(self.getXY(), heroPos) > misc.DistanceY(
                 self.getXY(), heroPos):
            if hX > sX:
        ...
        if misc.Distance(self.getXY(), heroPos) < 3:
            ...
            if i == 5:
                return self.move(random.choice(
                    ['up', 'down', 'left', 'right']), map, heroPos)
...
def newNpc(n, game):
    ...
    # enemies
        ...
        return skeletonKing(x, y, 'Skeleton King', 'skeleton.bmp',
            n, game.Director)
    else:
