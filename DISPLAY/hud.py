...
class hud():
    ...
    def update(self):
        ...
        (armor, weapon) = (self.game.myHero.getArmorEquipped(),
                           self.game.myHero.getWeaponEquipped())
        ...
        self.writeText(goldBox, (5, 17), '$' + str(
            self.game.myHero.getGold()), colors.white, colors.black, 10)
    ...
    def txtMessage(self, msg):
        ...
        self.scrollText[4] = '{}:{}.{} - {}'.format(
            self.game.Ticker.getHours() % 24,
            self.game.Ticker.getMins() % 60,
            self.game.Ticker.getSecs(), msg)
        if pygame.font:
    ...
    def npcMessage(self, message, img):
        ...
        for i in range(msgText.get_width() / 2):
            borderBox = pygame.Surface((((i * 2) + 60),
                msgText.get_height() + 20))
            borderBox.fill(colors.grey)
            borderBox.blit(img, (10, 10))
            borderBox.blit(msgText, (40, 10))
            self.screen.blit(borderBox, (const.gameBoardOffset + px +
                const.blocksize + (msgText.get_width() / 2) - i,
                const.gameBoardOffset + py + const.blocksize))
            pygame.display.flip()
    ...
    def npcDialog(self, message, img):
        ...
        for i in range(msgText.get_width() / 2):
            borderBox = pygame.Surface((((i * 2) + 60),
                (msgText.get_height() + ytext.get_height()) + 20))
            borderBox.fill(colors.grey)
            borderBox.blit(img, (10, 10))
            borderBox.blit(msgText, (40, 10))
            borderBox.blit(ytext, (40, 10 + msgText.get_height()))
            borderBox.blit(ntext, (
                borderBox.get_width() - ntext.get_width() - 20,
                10 + msgText.get_height()))
            self.screen.blit(borderBox, (
                const.gameBoardOffset + px + const.blocksize +
                (msgText.get_width() / 2) - i,
                const.gameBoardOffset + py + const.blocksize))
            pygame.display.flip()

        positions = [(40, 10 + msgText.get_height()),
                     (borderBox.get_width() - ntext.get_width() - 20,
                     10 + msgText.get_height())]
        yesPoints = [(40, 10 + msgText.get_height()),
                     ...
                     ytext.get_height()),
                     (40 + ytext.get_width(), 10 + msgText.get_height())]
        noPoints = [(borderBox.get_width() - ntext.get_width() - 20,
                    10 + msgText.get_height()),
                    (borderBox.get_width() - ntext.get_width() - 20,
                    10 + msgText.get_height() + ntext.get_height()),
                    (ntext.get_width() + borderBox.get_width() -
                     ntext.get_width() - 20, 10 + msgText.get_height() +
                     ntext.get_height()),
                    (ntext.get_width() + borderBox.get_width() -
                     ntext.get_width() - 20, 10 + msgText.get_height())]
            ...
            for e in pygame.event.get():
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_LEFT or e.key == pygame.K_RIGHT:
                        if selection:
                            pygame.draw.lines(borderBox, colors.gold, True,
                                pts[0], 1)
                            pygame.draw.lines(borderBox, colors.white, True,
                                pts[1], 1)
                        else:
                            pygame.draw.lines(borderBox, colors.gold, True,
                                pts[1], 1)
    ...
    def boxMessage(self, message):
        ...
        (px, py, px2, py2) = self.game.myHero.getRect()
        for i in range(msgText.get_width() / 2):
            borderBox = pygame.Surface((((i * 2) + 20),
                msgText.get_height() + 20))
            borderBox.fill(colors.grey)
