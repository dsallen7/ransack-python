...
class effects():
    ...
    def scrollFromCenter(self, screenOne, screenTwo):
        self.screen.blit(screenOne, (const.gameBoardOffset, const.gameBoardOffset) )
        for i in range( screenTwo.get_width()/2 ):
            screenOne.blit(screenTwo, # source 
                           ( (screenOne.get_width()/2)-i, const.gameBoardOffset ), # dest
                           ( (screenOne.get_width()/2)-i, const.gameBoardOffset, 2*i, 300)
                           )
            #self.clock.tick(100)
            self.screen.blit(screenOne, (const.gameBoardOffset, const.gameBoardOffset) )
            pygame.display.flip()