...
from entity.maps import generalmap, tile
from entity.maps.ROOM import room, roomtools
from UTIL import const, colors, misc

from script import enemy as enemyScr, map as mapScr
...
class Generator():
    ...
    def __init__(self, DIM, level=1):
        self.generatedMap = generalmap.genMap(DIM, level, 'Dungeon Level '+str(level) )
        self.generatedMap.type = 'dungeon'
        self.copyText = []
        self.level = level
    ...
    def branchTile(self, thisMap, room):
        (xpos, ypos) = room.getPos()
        (xdim, ydim) = room.getDimensions()
        candidateList = [ ]
        #room.printGrid()
        for i in range(1,xdim-1):
            #print thisMap.getEntry(i+xpos,ypos), room.getGrid(i,0).getFG()
            if thisMap.getEntry(i+xpos,ypos) == const.EWWALL:
                candidateList += [(i+xpos,ypos)]
            #print thisMap.getEntry(i+xpos,ypos+ydim-1), room.getGrid(i,ydim-1).getFG()
            if thisMap.getEntry(i+xpos,ypos+ydim-1) == const.EWWALL:
                candidateList += [(i+xpos,ypos+ydim-1)]
        for i in range(1,ydim-1):
            #print thisMap.getEntry(xpos,i+ypos), room.getGrid(0,i).getFG()
            if thisMap.getEntry(xpos,i+ypos) == const.NSWALL:
                candidateList += [(xpos,i+ypos)]
            #print thisMap.getEntry(xpos+xdim-1,i+ypos), room.getGrid(xdim-1,i).getFG()
            if thisMap.getEntry(xpos+xdim-1,i+ypos) == const.NSWALL:
    ...
    def checkNewRoom(self, thisMap, newRoom, tile, dir):
        (xpos, ypos) = tile
        (xdim, ydim) = newRoom.getDimensions()
        if xpos + xdim + 1 >= thisMap.DIM or ypos + ydim + 1 >= thisMap.DIM:
            return False
        ...
        for i in range(newRoomPos[0], newRoomPos[0]+xdim):
            for j in range(newRoomPos[1], newRoomPos[1]+ydim):
                if thisMap.getEntry(i, j) != const.VOID:
                    return False
    ...
    def generateMap(self, maxRooms):
        ...
        startingRoom.setPos( ( (self.generatedMap.DIM/2)-(xdim/2), (self.generatedMap.DIM/2)-(ydim/2) ) )
        self.rooms += [startingRoom]
        self.addRoom(self.generatedMap, startingRoom, startingRoom.getPos() )
        self.secretRooms = []
        ...
        while len(self.rooms) < maxRooms:
            ...
            tile, dir = self.branchTile(self.generatedMap, candidateRoom)
            ...
            if self.checkNewRoom(self.generatedMap, newRoom, tile, dir):
                startingRoom.setPos( ( (self.generatedMap.DIM/2)-(xdim/2), (self.generatedMap.DIM/2)-(ydim/2) ) )
            # 8  if yes - continue, if not go back to step 4

            # 9 add new room to map and list of rooms
                self.rooms += [newRoom]
                self.addRoom(self.generatedMap, newRoom, newRoom.getPos() )
        ...
        self.generatedMap.setEntry( xpos + xdim/2, ypos + ydim/2, 120)
        ...
        self.generatedMap.setEntry( xpos + xdim/2, ypos + ydim/2, const.STAIRDN, len(self.rooms))
        self.generatedMap.setEntry( doorX, doorY, const.DOOR, len(self.rooms))
        self.generatedMap.pointOfExit = ( xpos + xdim/2, ypos + ydim/2 )

        # up
        choice2 = choice1.neighbors[0]
        (xpos, ypos) = choice2.getPos()
        (xdim, ydim) = choice2.getDimensions()
        self.generatedMap.setEntry( xpos + xdim/2, ypos + ydim/2, const.STAIRUP, len(self.rooms))
        self.generatedMap.pointOfEntry = ( xpos + xdim/2, ypos + ydim/2 )
        self.generatedMap.heroStart = ( xpos + xdim/2, ypos + ydim/2 )
        ...
        self.generatedMap.hs = ( xpos + xdim/2, ypos + ydim/2 )
        ...
        for i in range(maxRooms/5):
            ...
            self.generatedMap.setEntry( xpos + xdim/2, ypos + ydim/2, const.CHEST, len(self.rooms))
        ...
        self.generatedMap.chests = dict(chestlist)
        # add enemies
        while len(self.rooms) > 1:
            room = choice(self.rooms)
            (xpos, ypos) = room.getPos()
            (xdim, ydim) = room.getDimensions()
            self.generatedMap.NPCs.append( (( xpos + xdim/2,
                                     ypos + ydim/2),
                                   choice(enemyScr.enemiesByDungeonLevel[self.generatedMap.level] ) ) )
        ...
        self.generatedMap.setEntry( xpos + xdim/2, ypos + ydim/2, const.KEY, len(self.rooms))
        self.rooms.remove(keyRoom)

    def addRoom(self, thisMap, room, pos):
        room.serial = len(self.rooms)
        thisMap.copyText = room.grid
        thisMap.mapPaste( pos, len(self.rooms) )

    def getMapBall(self):
        return self.generatedMap.getMapBall()
