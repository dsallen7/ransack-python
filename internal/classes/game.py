...
class game():
    ...
    def __init__(self, screen, clock, FX, loadTicker=None, loadHero=None,
            loadDungeon=None, loadDirector=None, currentMap=2, levelDepth=0):
    ...
    def nextLevel(self):
        ...
        if self.currentMap == len(self.myDungeon):
            ...
            if self.levelDepth == 10:
                ...
                if (self.levelDepth % 5) == 0:
                    self.myDungeon.append(self.generateMap(40,
                                          self.levelDepth, type='maze'))
                else:
                    self.myDungeon.append(self.generateMap(40,
                                          self.levelDepth, type='dungeon'))
                self.boxMessage('Now entering dungeon level {}'.format(
                    self.levelDepth))
            self.myMap = self.myDungeon[self.currentMap]
            ...
            if self.myMap.getType() == 'dungeon':
                ...
                self.boxMessage('Now entering {}'.format(self.myMap.getType()))
    ...
    def screenShot(self):
        """takes screen shot and saves as bmp in serial fashion,
        beginning with 1
        """
        serial = 1
        while os.access("ransack{}.bmp".format(serial), os.F_OK):
            serial += 1
        pygame.image.save(self.screen, "ransack{}.bmp".format(serial))
    ...
    def event_handler(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                # cast spell
                self.myHero.castSpell(self.myMenu.invMenu(
                    self.myHero.getSpells(), "Spells:"), self)
            elif event.key == pygame.K_s:
                # show stats
                self.myMenu.displayHeroStats(self.myHero)
            elif event.key == pygame.K_i:
                # use item
                self.myHero.useItem(self.myMenu.invMenu(
                    self.myHero.getItems(), "ITems:"), self)
            elif event.key == pygame.K_w:
                # equip weapon
                self.myHero.equipWeapon(self.myMenu.invMenu(
                    self.myHero.getWeapons(), "Weapons:"))
            elif event.key == pygame.K_a:
                # equip armor
                self.myHero.equipArmor(self.myMenu.invMenu(
                    self.myHero.getArmor(), "Armor:"))
    ...
    def mouseHandler(self, event):
        ...
        if ((const.gameBoardOffset <= mx <
                const.gameBoardOffset + self.gameBoard.get_width()) and
                (const.gameBoardOffset <= my <
                const.gameBoardOffset + self.gameBoard.get_height())):
            # map handler
            pass
        elif ((const.gameBoardOffset + self.gameBoard.get_width() < mx <=
                const.gameBoardOffset + self.gameBoard.get_width() + 150) and
                (const.gameBoardOffset < my <= const.gameBoardOffset + 300)):
            # hud handler
            self.myHud.mouseHandler(event,
                 mx - (const.gameBoardOffset + self.gameBoard.get_width()),
                 my - const.gameBoardOffset)
    ...
    def move(self, direction):
        ...
        for n in self.NPCs:
            if ((X + moveX) / const.blocksize,
                    (Y + moveY) / const.blocksize) == n.getXY():
                return
        i = self.myMap.getEntry((X + moveX) / const.blocksize,
         (Y + moveY) / const.blocksize)
        # detect blocking tiles first, otherwise they will be ignored
        # stores
        if i == const.BLKSMDOOR:
            self.Blacksmiths[self.myMap.shops[((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize)][1]].enterStore(self.myHero)
            return
        if i == const.ARMRYDOOR:
            self.Armories[self.myMap.shops[((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize)][1]].enterStore(self.myHero)
            return
        if i == const.ITEMSDOOR:
            self.Itemshops[self.myMap.shops[((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize)][1]].enterStore(self.myHero)
            return
        if i == const.MAGICDOOR:
            self.Magicshops[self.myMap.shops[((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize)][1]].enterStore(self.myHero)
        if i == const.TAVRNDOOR:
            return self.Tavern.enterStore(self.myHero, self)
        if i == const.EWDOOR:
            self.myMap.setEntry((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize, const.EWDOORO)
            self.Display.redrawXMap(self.myMap)
        if i == const.NSDOOR:
            self.myMap.setEntry((X + moveX) / const.blocksize,
             (Y + moveY) / const.blocksize, const.NSDOORO)
            self.Display.redrawXMap(self.myMap)
        ...
        if i == const.DOOR:
            if self.myHero.getPlayerStats()[8] == 0:
                ...
                self.myHero.takeKey()
                self.myMap.setEntry((X + moveX) / const.blocksize,
                    (Y + moveY) / const.blocksize, self.myMap.defaultBkgd)
        #item
        ...
        if i == const.STAIRDN:
            self.nextLevel()
            self.Display.drawSprites(self.myHero, self.myMap,
                                     self.gameBoard, self, animated=False)
            return
        # Stairs up
        if i == const.STAIRUP:
            self.prevLevel()
            ...
            return
        # open space
        if ((0 <= X + moveX < const.blocksize * self.myMap.getDIM())
                and (0 <= Y + moveY < const.blocksize * self.myMap.getDIM())
                and i in range(24)):
    ...
    def launchBattle(self, mName, lD):
        ...
        if g is True:
            ...
            # won battle
            self.textMessage('You find {} gold pieces!'.format(g))
    ...
    def getSaveBall(self):
        saveBall = (self.Ticker, self.myHero.getSaveBall(), self.myDungeon,
                    self.Director, self.currentMap, self.levelDepth)
        return saveBall
    ...
    def displayGameBoard(self):
        ...
        if self.myMap.type == 'dungeon':
            pass
            #self.Display.drawShade(self.myMap, self.gameBoard)
        self.screen.blit(self.gameBoard, (const.gameBoardOffset,
    ...
    def mainLoop(self):
        gameFrame, gameFrameRect = load_image.load_image(
            'gamescreen600.bmp', None)
        ...
        while self.gameOn:
            ...
            if self.updateNPCs() or self.myHero.moving:
                self.Display.drawSprites(self.myHero, self.myMap,
                                         self.gameBoard, self,
                                         self.myHero.dir, animated=True)
            else:
                self.Display.drawSprites(self.myHero, self.myMap,
                                         self.gameBoard, self, None,
                                         animated=True)
