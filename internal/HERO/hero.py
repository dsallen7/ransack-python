...
class hero(pygame.sprite.Sprite):
    ...
    def __init__(self, load=None):
        ...
        if self.gender == 'Male':
            ...
            self.images = images.loadNPC('fherosheet.bmp')
        self.imgIdx = 2
        self.image = self.images[self.imgIdx]
        self.rect = (const.blocksize,
                     const.blocksize,
                     const.blocksize,
                     const.blocksize)
    ...
    def getPlayerStats(self):
        return (self.currHP, self.maxHP, self.currMP, self.maxMP,
            self.strength, self.dex, self.intell, self.score, self.keys,
    ...
    def getItem(self, item):
        ...
        if item.getType() == const.KEY:
            ...
            return self.gainWeapon(item.getType(), item.getLevel(),
                                  [item.plusStr, item.plusItl, item.plusDex])
        elif item.getType() in [31, 32, 33]:
            return self.gainArmor(item.getType(), item.getLevel())
        elif (item.getType() == const.SPELLBOOK
                or item.getType() == const.PARCHMENT):
            pass
    ...
    def equipArmor(self, armor):
        if armor is None:
            return
        if self.armorEquipped[armor.getType() - 31] is None:
            self.armorEquipped[armor.getType() - 31] = armor
            self.armorClass += (armor.getLevel() + 1) ** 2
        else:
            self.armorClass -= (self.armorEquipped[armor.getType() - 31]
                .getLevel() + 1) ** 2
            self.armorClass += (armor.getLevel() + 1) ** 2
    ...
    def castSpell(self, spell, game, battle=False, item=False):
        if spell is None:
            #game.textMessage('')
            return
        elif (spell.getType() in [1, 2] and battle is False):
            game.textMessage('That spell may only be cast in battle')
            return False
        elif (spell.getType() in [3] and battle is True):
        ...
        if spell.getType() == const.TLPT:
            ...
            game.Display.drawSprites(self, game.myMap, game.gameBoard,
                game, animated=False)
            ...
            game.textMessage('The fireball hits the monster for '
               + str(dmg) + ' points!')
            return dmg
            ...
            game.Ticker.tick(spell.getCastTime())
            dmg = random.randrange(self.intell, 2 * self.intell)
            game.textMessage('The iceball hits the monster for ' + str(dmg)
                + ' points!')
            return dmg
        '''
        if item == False:
    ...
    def getSaveBall(self):
        ...
        sln = self.slain
        return (gnd, str, itl, dex, X, Y, cHP, mHP, cMP, mMP, scr, kys, lvl,
            cXP, nXP, wpn, weq, arm, aeq, itm, spl, gld, sts, sln)

    def installLoadedHero(self, load):
        (gnd, str, itl, dex, X, Y, cHP, mHP, cMP, mMP, scr, kys, lvl, cXP,
            nXP, wpn, weq, arm, aeq, itm, spl, gld, sts, sln) = load
        self.gender = gnd
        self.strength = str
        self.intell = itl
        self.dex = dex
    ...
    def showLocation(self):
        ...
        if pygame.font:
            ...
            locText = font.render("Self.X:" + str(self.X) + "Self.Y:"
                + str(self.Y) + "RectX:" + str(x1) + "RectY" + str(y1),
                1, colors.red, colors.yellow)
