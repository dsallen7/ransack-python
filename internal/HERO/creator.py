...
class Creator():
    ...
    def __init__(self):
        ...
        self.sliders = [slider.Slider(216, 145),
            slider.Slider(172, 131), slider.Slider(101, 61, 0, 1)]
        self.gender = 'male'
    ...
    def displayStat(self, stat, loc):
        (x, y) = loc
        statT = text.Text(stat, os.getcwd() + "/FONTS/devinne.ttf", 18,
            colors.white, colors.black, transparent=True)
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    if (150 + sldr.getYLoc() <= mY <= 150 + sldr.getYLoc()
                            + sldr.getMax() * 10):
    ...
    def calculateStats(self):
        # goes up with
        self.HP = self.baseHP + (self.sliders[0].getMax() -
            self.sliders[0].getValue()) * 10
        # goes down with
        self.MP = self.baseMP + self.sliders[0].getValue() * 10
        self.str = self.bStr + ((((self.sliders[0].getMax() -
             self.sliders[1].getValue()) * 10) / 2) / 5) + 2 * (1 -
             self.sliders[2].getValue())
        self.itl = self.bItl + ((self.sliders[1].getValue() * 10) / 4) / 5
        self.dex = self.bDex + (((self.sliders[1].getValue() * 10)
            / 2) / 5) + 2 * self.sliders[2].getValue()
    ...
    def updateDisplay(self, screen):
        ...
        for s in self.sliders:
            self.displayField.blit(self.cursor, (
                s.getXLoc(), s.getYLoc() + (s.getValue() * 10)))
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    (mX, mY) = pygame.mouse.get_pos()
                    for s in self.sliders:
                        if (153 + s.getXLoc() <= mX < 171 + s.getXLoc()
                                and 145 + s.getYLoc() + (s.getValue() * 10)
                                <= mY < 161 + s.getYLoc() +
                                (s.getValue() * 10)):
                            self.slide(s, screen)
                    # done
                    if 291 <= mX < 340 and 387 <= mY < 416:
                        return (self.gender,
                                ...
                                # score, keys
                                1, 0, 20,
                                # level, current XP, XP for next lev
                                [], None,
                                #weapons, eq. weapons
                                [], [None, None, None],
                                #armor, eq. armor
