...
boxPointsFn = lambda x: ((x[0], x[1]), (x[0], x[1] + const.blocksize),
    (x[0] + const.blocksize, x[1] + const.blocksize),
    (x[0] + const.blocksize, x[1]))
...
class menu():
    ...
    def openWindow(self, xDim, yDim):
        for i in range(xDim / 2):
            ...
            self.screen.blit(borderBox, ((self.screen.get_size()[0] / 2) - i,
                100))
            pygame.display.flip()
        return borderBox
                ...
                if pygame.font:
                    ...
                    if hasattr(item, "__iter__"):
                        if item[0].name in ['item', 'spellbook', 'parchment']:
                            msgText = font.render('x' + str(item[0].qty),
                                1, colors.white, colors.black)
                            itemBox.blit(msgText, (17, 17))
                        else:
                            msgText = font.render(
                                'L' + str(item[0].getLevel()),
                                1, colors.white, colors.black)
                            itemBox.blit(msgText, (17, 17))
                    else:
                        if item.name in ['item', 'spellbook', 'parchment']:
                            ...
                            msgText = font.render('L' + str(item.getLevel()),
                                1, colors.white, colors.black)
                            itemBox.blit(msgText, (17, 17))
    ...
    def displayChest(self, chest):
        ...
        #draw available items in window
        w = 10
        #var to draw items across screen
        availableItems = []
        hPosList = [10]
        for i in chest:
            if len(i) == 2:
            ...
            if pygame.font:
                ...
                msgText = font.render('x' + str(num), 1,
            ...
            w += const.blocksize
        hPos = 10
        #horizontal position of selection box
        boxPointsFn = lambda x: ((x, const.blocksize),
                                (x, 2 * const.blocksize),
                                (x + const.blocksize, 2 * const.blocksize),
                                (x + const.blocksize, const.blocksize))
        boxPoints = boxPointsFn(hPos)
        pygame.draw.lines(menuBox, colors.white, True, boxPoints, 1)

        self.screen.blit(menuBox,
            ((self.screen.get_size()[0] / 2) - (188 / 2), 100))
        pygame.display.flip()
    ...
    def mainMenu(self, hero, calledMenu):
        """
        Input:  list of items to display in menu,
    ...
    def displayHeroStats(self, hero):
        ...
        if pygame.font:
            ...
            statsBox.blit(msgText, ((statsBox.get_width() / 2) - (
            msgText.get_width() / 2), 10))
        ...
        statsBox.blit(font.render('HP: ' + str(hero.currHP) + '/' +
            str(hero.maxHP), 1, colors.white, colors.black),
            ((statsBox.get_width() / 4), 30))
        statsBox.blit(font.render('MP: ' + str(hero.currMP) + '/' +
            str(hero.maxMP), 1, colors.white, colors.black),
            ((statsBox.get_width() / 2), 30))
        statsBox.blit(font.render('Str: ' + str(stats[4]) + ' Int: ' +
            str(stats[6]) + ' Dex: ' + str(stats[5]), 1, colors.white,
            colors.black), (statsBox.get_width() / 4, 70))
        statsBox.blit(font.render('Armor: ' + str(hero.armorClass), 1,
            colors.white, colors.black), (statsBox.get_width() / 4, 90))
        statsBox.blit(font.render('Level: ' + str(hero.level), 1, colors.white,
            colors.black), (statsBox.get_width() / 4, 110))
        statsBox.blit(font.render('Monsters slain: ' + str(hero.slain), 1,
            colors.white, colors.black), (statsBox.get_width() / 4, 150))
        self.screen.blit(statsBox, ((self.screen.get_width() / 2) -
            (statsBox.get_width() / 2), 100))
        pygame.display.flip()
        while (pygame.event.wait().type != pygame.KEYDOWN):
            pass
    ...
    def invMenu(self, items, text):
        ...
        if pygame.font:
            ...
            menuBox.blit(msgText, ((menuBox.get_width() / 2) -
                (msgText.get_width() / 2), 10))

        #draw available items in window
        itemsBox, availableItems = self.menuWin(items)
        menuBox.blit(itemsBox, ((menuBox.get_width() / 2) -
            (itemsBox.get_width() / 2), 40))
        selection = 0
        ...
        while True:
            ...
            if numItems >= 1:
                if hasattr(availableItems[selection], "__iter__"):
                    descBox = font.render(
                        availableItems[selection][0].getDesc(), 1,
                        colors.white, colors.gold)
                else:
                    descBox = font.render(
                        availableItems[selection].getDesc(), 1,
                        colors.white, colors.gold)
                menuBox.blit(descBox, ((menuBox.get_width() / 2) -
                    (descBox.get_width() / 2), 30))
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    pygame.draw.lines(itemsBox, colors.black, True,
                        boxPoints, 1)
                    ...
                    if event.key == pygame.K_DOWN:
                        if hasattr(items[selection], "__iter__"):
                            items[selection] = items[selection][1:] + [items[
                                selection][0]]
                            itemsBox, availableItems = self.menuWin(items)
                            menuBox.blit(itemsBox, ((menuBox.get_width() / 2)
                                - (itemsBox.get_width() / 2), 40))
                            pygame.draw.lines(itemsBox, colors.white, True,
                                boxPoints, 1)
                            copyBox = menuBox
                        else:
                            pass
                    if event.key == pygame.K_UP:
                        if hasattr(items[selection], "__iter__"):
                            items[selection] = [items[selection][
                                len(items[selection]) - 1]] + \
                                items[selection][:len(items[selection]) - 1]
                            itemsBox, availableItems = self.menuWin(items)
                            menuBox.blit(itemsBox, ((menuBox.get_width() / 2)
                                - (itemsBox.get_width() / 2), 40))
                            pygame.draw.lines(itemsBox, colors.white, True,
                    ...
                    if event.key == pygame.K_RETURN:
                        if availableItems == []:
                            ...
                            return availableItems[selection]
            cursorPos = positions[selection]
            boxPoints = boxPointsFn(cursorPos)
            pygame.draw.lines(itemsBox, colors.white, True, boxPoints, 1)
            menuBox.blit(itemsBox, ((menuBox.get_width() / 2) -
                (itemsBox.get_width() / 2), 40))
            self.screen.blit(menuBox, ((self.screen.get_size()[0] / 2) -
                (200 / 2), 100))
            pygame.display.flip()
    ...
    def storeMenu(self, items, text, prices):
        ...
        if pygame.font:
            font = pygame.font.Font(os.getcwd() + "/FONTS/courier.ttf", 14)
            msgText = font.render(text, 1, colors.white, colors.gold)
            menuBox.blit(msgText,
                ((menuBox.get_width() / 2) - (msgText.get_width() / 2), 10))
        ...
        self.screen.blit(menuBox,
            ((self.screen.get_size()[0] / 2) - (200 / 2), 100))
        pygame.display.flip()
        numItems = len(availableItems)
        copyBox = menuBox
        # wait for selection
        while True:
            ...
            if availableItems[selection].getName() == 'item':
                priceText = font.render(availableItems[selection].getDesc() +
                      ' $' + str(prices[availableItems[selection].getType()]),
                      1, colors.white, colors.gold)
            elif (availableItems[selection].getName() == 'spellbook' or
                    availableItems[selection].getName() == 'parchment'):
                priceText = font.render(availableItems[selection].getDesc() +
                     ' $' + str(prices[(availableItems[selection].getType(),
                     availableItems[selection].getLevel(),
                     availableItems[selection].getSpellNum())]), 1,
                     colors.white, colors.gold)
            elif (availableItems[selection].getName() == 'armor' or
                    availableItems[selection].getName() == 'weapon'):
                priceText = font.render(availableItems[selection].getDesc() +
                    ' $' + str(prices[(availableItems[selection].getType(),
                    availableItems[selection].getLevel())]), 1,
                    colors.white, colors.gold)
            menuBox.blit(priceText, ((menuBox.get_width() / 2) -
                (priceText.get_width() / 2), 20))
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    pygame.draw.lines(itemsBox, colors.black,
                        True, boxPoints, 1)
                    if event.key == pygame.K_RIGHT:
